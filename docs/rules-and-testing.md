...
- Фото/attachments отложены до post-deploy этапа.
- Запланированные лимиты для будущей реализации: до 3 фото на запись и до 2 MB на фото после сжатия.
- Будущая реализация будет online-only и не будет входить в offline-sync.

## Offline

- Offline можно создавать и редактировать клиентов.
- Offline можно создавать, редактировать и отменять записи.
- Фото/attachments не входят в pre-deploy offline scope.
- Синхронизация выполняется через local outbox и `/api/sync/push`, `/api/sync/pull`.

## Backup

- Backup выполняется вручную.
- Архив должен включать SQLite database и конфигурацию деплоя. После реализации attachments он также должен включать uploads.
...
- post-deploy attachments upload с проверкой лимитов.
